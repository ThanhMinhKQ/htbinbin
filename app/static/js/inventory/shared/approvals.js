export default function(config) {
    ...
    async fetchApprovals(page = 1) {
        if (!config.canFetch(this)) return;
        ...
        let url = config.buildApprovalUrl(this, statusParam, page);
    ...
    async openApprovalModal(t) {
        ...
        if (!config.refreshApprovalDetail) {
            this.populateApprovalForm(t);
            return;
        }

        this.isLoadingApprovalDetails = true;
        try {
    ...
    // --- RECEIPT/INBOUND ---
    openReceiptModal(t) {
        const status = this.normalizeTicketStatus(t?.status);
        if (!t || status !== 'SHIPPING') return;
    ...
    async submitReceipt() {
        ...
        if (this.normalizeTicketStatus(this.receivingTicket.status) !== 'SHIPPING') {
