export function masterDataApp() {
    return {
        ...
        pForm: {
            ...
            cost_price: 0, sell_price: 0, is_active: true, is_sellable: false
        },
        tempPackPrice: '',
        ...
        async quickToggleSell(product) {
            const newVal = !product.is_sellable;
            // If enabling sell but no price, prompt for price first
            if (newVal && (!product.sell_price || product.sell_price <= 0)) {
                const price = prompt(`Nhập giá bán lẻ cho "${product.name}" (VNĐ):`, '');
                if (price === null) return; // cancelled
                const parsed = parseInt(price.replace(/[^0-9]/g, '')) || 0;
                if (parsed <= 0) {
                    alert('Giá bán phải lớn hơn 0 để kích hoạt bán tại PMS!');
                    return;
                }
                product.sell_price = parsed;
            }
            product.is_sellable = newVal;
            await this._patchProductSell(product);
        },

        async quickSetSellPrice(product) {
            const current = product.sell_price ? new Intl.NumberFormat('vi-VN').format(product.sell_price) : '';
            const price = prompt(`Giá bán lẻ cho "${product.name}" (VNĐ):`, current);
            if (price === null) return;
            const parsed = parseInt(price.replace(/[^0-9]/g, '')) || 0;
            product.sell_price = parsed;
            if (parsed > 0 && !product.is_sellable) {
                if (confirm('Tự động bật "Cho phép bán tại PMS"?')) {
        ...
        async _patchProductSell(product) {
            try {
                const payload = {
                    name: product.name,
                ...
                if (!res.ok) {
                    ...
                    await this.fetchProducts(); // revert
                }
            } catch (e) {
                alert('Lỗi kết nối');
                await this.fetchProducts();
