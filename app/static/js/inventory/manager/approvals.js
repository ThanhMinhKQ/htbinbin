export default {
    async fetchApprovals(page = 1) {
        ...
        // [CHANGE] Use source_warehouse_id for Approvals (Requests *FROM* my warehouse)
        let url = `/api/inventory/requests?source_warehouse_id=${this.currentWarehouseId}&status=${statusParam}&page=${page}&per_page=${this.perPage}`;
    ...
    // --- APPROVAL ACTION ---
    async openApprovalModal(t) {
        // 1. Show Modal immediately with Loading State
        this.approvingTicket = t; // Basic info for header
        this.isLoadingApprovalDetails = true; // Show spinner for content

        // 2. Fetch Fresh Data from Server
        try {
            const res = await fetch(`/api/inventory/request/${t.id}`);
            if (res.ok) {
                const freshTicket = await res.json();
                this.approvingTicket = freshTicket;
                this.populateApprovalForm(freshTicket);
            }
        } catch (e) {
            console.error("Failed to refresh approval ticket details", e);
            // Fallback to existing data if fetch fails
            this.populateApprovalForm(t);
        } finally {
            this.isLoadingApprovalDetails = false;
        }
    },

    populateApprovalForm(t) {
        ...
        this.approvalForm.approver_notes = t.approver_notes || '';
    ...
    async handleReceiptImageUpload(event) {
        ...
        for (const file of files) {
            ...
            try {
                ...
                // Assuming it's mixed in index.js (utils is mixed in)
                ...
                this.receiptForm.images.push({
                    ...
                    compressed: !!(compressedFile && compressedFile.size < file.size)
                });
