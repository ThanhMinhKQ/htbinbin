...
// ─── Load guest data into form (shared between agEditGuest and agFillFromScan DB path) ───

async function agLoadGuestIntoForm(guest) {
    const nameEl   = document.getElementById('ag-name');
    const genderEl = document.getElementById('ag-gender');
    const natEl    = document.getElementById('ag-nationality');
    const cccdEl   = document.getElementById('ag-cccd');
    ...
    const notesEl  = document.getElementById('ag-notes');
    const idTypeEl = document.getElementById('ag-id-type');
    const idExpEl  = document.getElementById('ag-id-expire');
    const birthEl  = document.getElementById('ag-birth');
    ...
    if (addrEl)   addrEl.value   = guest.address || '';
    ...
    // Address: detect mode
    const hasOldData = !!(guest.old_city || guest.old_ward);
    const addrType = hasOldData ? 'old' : 'new';
    const oldRadio = document.querySelector('input[name="ag-area"][value="old"]');
    const newRadio = document.querySelector('input[name="ag-area"][value="new"]');
    const distGrp  = document.getElementById('ag-grp-district');
    ...
    if (addrType === 'old') {
        ...
        if (wEl && wEl.value) wEl.dispatchEvent(new Event('blur'));
        if (dEl) dEl.dispatchEvent(new Event('blur'));
        if (pEl) pEl.dispatchEvent(new Event('blur'));
    } else {
        if (typeof agLoadNewProvinces === 'function') {
    ...
    // Lock all fields for existing guest
    agLockAllFields();
    ...
    if (idExpEl) {
        const expGrp = document.getElementById('ag-grp-expire');
        if (expGrp && expGrp.style.display !== 'none') idExpEl.readOnly = false;
    }
    document.querySelectorAll('input[name="ag-invoice"]').forEach(r => r.disabled = false);
    ['ag-tax-code', 'ag-tax-contact'].forEach(id => {
        const el = document.getElementById(id);
    ...
    if (typeof agShowAutofillNotice === 'function') {
        agShowAutofillNotice(guest.full_name, guest.cccd);
    }
}
window.agLoadGuestIntoForm = agLoadGuestIntoForm;
...
async function agFillFromScan(parsed) {
    ...
    // ── Step 1: Check if CCCD exists ────────────────────────────────────
    const cccdToCheck = idValue.trim().toUpperCase();
    if (cccdToCheck && cccdToCheck.length >= 3) {
        // First check local list (existing guests in this stay)
        const localIdx = agGuestList.findIndex(
            g => (g.cccd || '').trim().toUpperCase() === cccdToCheck
        );
        if (localIdx !== -1) {
            // Guest already in this stay's list — switch to edit mode
            await agEditGuest(localIdx);
            if (cccdEl) cccdEl.value = cccdToCheck;
            pmsToast(`Đã chọn khách: ${agGuestList[localIdx].full_name || cccdToCheck}`, true);
            return;
        }
        ...
        try {
            ...
            if (searchRes && searchRes.guests && searchRes.guests.length > 0) {
                ...
                // DB guest (previous stay) — fill form with their data, keep "Thêm Khách" toolbar.
                // Same behavior as pms_checkin.js: toolbar stays, form is prefilled.
                // When user clicks "Thêm Khách", submitAG POSTs to add them to current stay.
                // Backend resolves Guest master by CCCD and creates new HotelGuest for this stay.
                await agLoadGuestIntoForm(guest);
                agGuestId = guest.id || null;
                _agIsOldGuest = true;
                _agEditIndex = null;
                document.getElementById('ag-guest-id').value = agGuestId || '';
                // DO NOT call agSetEditMode — toolbar stays as "Thêm Khách"
                agRenderGuestList();
                pmsToast(`Đã tìm thấy khách: ${guest.full_name || cccdToCheck}`, true);
                return;
            }
        } catch (e) {
            console.warn('[AG] Scan: DB lookup failed, filling from scan data:', e);
        }
    }

    // ── Step 2: No DB match — fill from scan, unlock fields ─────────────
    if (cccdEl) {
