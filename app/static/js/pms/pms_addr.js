...
const PMS_ADDR = {
    ...
    _isInDatalist(dl, value) {
        ...
        const v = value.trim();
        const vNorm = _addrNormVietnamese(_addrNormWardNumber(v));
        for (const opt of dl.options) {
            const optNorm = _addrNormVietnamese(_addrNormWardNumber(opt.value.trim()));
            if (optNorm === vNorm) return true;
            // Also check stripped versions
            const strippedOpt = this.stripPrefix(opt.value).toLowerCase();
            const strippedVal = this.stripPrefix(v).toLowerCase();
            if (_addrNormVietnamese(_addrNormWardNumber(strippedOpt)) === _addrNormVietnamese(_addrNormWardNumber(strippedVal))) return true;
        }
        return false;
    },
    ...
    _findSuggestions(dl, value) {
        ...
        for (const opt of dl.options) {
            const optNorm = _addrNormVietnamese(_addrNormWardNumber(opt.value.trim()));
            // Starts with
            if (optNorm.startsWith(vNorm)) {
                suggestions.push(opt.value);
            }
            // Contains
            else if (optNorm.includes(vNorm)) {
                suggestions.push(opt.value);
            }
        }
...
// Internal helper: Vietnamese normalization for comparison
function _addrNormVietnamese(s) {
    if (!s) return '';
    return String(s)
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
...
function _addrNormWardNumber(s) {
    if (!s) return s;
    return s.replace(/^(phường|xã|thị trấn)\s*0+(\d+)/i, (match, prefix, num) => prefix + ' ' + parseInt(num, 10));
}
...
function addrIsInDatalist(datalistId, value) {
    ...
    const dl = document.getElementById(datalistId);
    if (!dl) return false;
    const vNorm = _addrNormVietnamese(_addrNormWardNumber(value.trim()));
    for (const opt of dl.options) {
        const optNorm = _addrNormVietnamese(_addrNormWardNumber(opt.value.trim()));
        if (optNorm === vNorm) return true;
        // Also check stripped versions
        const strippedOpt = PMS_ADDR.stripPrefix(opt.value).toLowerCase();
        const strippedVal = PMS_ADDR.stripPrefix(value).toLowerCase();
        if (_addrNormVietnamese(_addrNormWardNumber(strippedOpt)) === _addrNormVietnamese(_addrNormWardNumber(strippedVal))) return true;
    }
    return false;
}
