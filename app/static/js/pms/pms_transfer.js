...
// ─────────────────────────────────────────────────────────────────────────────
// Select Room — fetch old charge preview, autofill fee input
// ─────────────────────────────────────────────────────────────────────────────
async function pmsTfSelectRoom(roomId, roomNum, price, typeName) {
    // Deselect all
    ...
    // Enable confirm
    const btnEl = document.getElementById('tf-submit-btn');
    if (btnEl) btnEl.disabled = true; // re-disable while loading

    // Fetch old charge from checkout preview
    ...
    try {
        ...
        if (feeInput) feeInput.value = Math.round(oldCharge);

        const noteInput = document.getElementById('tf-transfer-note-input');
        if (noteInput) noteInput.value = '';

        pmsTfUpdateHint();
        _tfShowChargePreview();
    } catch (_) {
        // Nếu không lấy được preview, vẫn cho phép chuyển phòng không có phí
        window._pmsTfOldCharge = 0;
        const feeInput = document.getElementById('tf-transfer-fee-input');
...
// ─────────────────────────────────────────────────────────────────────────────
// Update hint khi user thay đổi fee input
// ─────────────────────────────────────────────────────────────────────────────
function pmsTfUpdateHint() {
    const hintEl = document.getElementById('tf-charge-hint');
    if (!hintEl) return;

    const feeInput = document.getElementById('tf-transfer-fee-input');
    const fee = parseFloat(feeInput?.value || '0') || 0;
    const newPrice = window._pmsTfNewPrice || 0;
    const oldCharge = window._pmsTfOldCharge || 0;

    if (!window.pmsTfSelectedId) {
        hintEl.textContent = '';
        hintEl.className = 'tf-charge-hint';
        return;
    }

    if (fee === 0) {
        hintEl.textContent = 'Không phụ thu phí chuyển phòng';
        hintEl.className = 'tf-charge-hint same';
        return;
    }

    const diff = newPrice - oldCharge;
    if (diff > 0) {
        hintEl.textContent = `Phòng mới cao hơn ${diff.toLocaleString('vi-VN')}đ/đêm`;
        hintEl.className = 'tf-charge-hint up';
    } else if (diff < 0) {
        hintEl.textContent = `Phòng mới thấp hơn ${Math.abs(diff).toLocaleString('vi-VN')}đ/đêm`;
        hintEl.className = 'tf-charge-hint down';
    } else {
        hintEl.textContent = 'Cùng mức giá';
        hintEl.className = 'tf-charge-hint same';
    }
}
...
function _tfHideChargePreview() {
    ...
    if (noteInput) noteInput.value = '';
    const hintEl = document.getElementById('tf-charge-hint');
    if (hintEl) { hintEl.textContent = ''; hintEl.className = 'tf-charge-hint'; }
}
...
window.pmsTfUpdateHint   = pmsTfUpdateHint;
