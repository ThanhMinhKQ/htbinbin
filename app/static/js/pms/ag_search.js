...
async function agFillGuestFromOld(guest) {
    try {
        ...
        if (riskFlags.is_blacklisted || riskFlags.has_unpaid_debt) {
            const warnings = riskFlags.warnings?.length
                ? riskFlags.warnings.map(w => w.type === 'debt' ? `- Còn nợ: ${pmsMoney(w.amount || riskFlags.unpaid_debt_amount || 0)}` : `- ${w.message || 'Có cảnh báo CRM'}`).join('\n')
                : [
                    riskFlags.is_blacklisted ? '- Đang trong danh sách đen' : '',
                    riskFlags.has_unpaid_debt ? `- Còn nợ: ${pmsMoney(riskFlags.unpaid_debt_amount || 0)}` : '',
                  ].filter(Boolean).join('\n');
            if (!window.confirm(`Cảnh báo CRM cho khách "${guest.full_name || guest.cccd || ''}":\n\n${warnings}\n\nBạn vẫn muốn tiếp tục chọn khách này?`)) return;
        }
        _agIsOldGuest = true;
