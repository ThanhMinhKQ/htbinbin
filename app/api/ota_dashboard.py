...
from app.services.booking_service import BookingService, is_ota_like_booking  # type: ignore
...
GMAIL_FETCH_CONCURRENCY = 8
OTA_PROCESS_CONCURRENCY = 3
_running_manual_scans: set[str] = set()
...
def _email_sort_key(email: dict):
    try:
        parsed = parsedate_to_datetime(email.get("date") or "")
        timestamp = parsed.timestamp()
    except Exception:
        timestamp = 0
    return (timestamp, email.get("message_id") or email.get("uid") or "")


def _build_ota_change_token(bookings, cancelled_count: Optional[int] = None) -> Dict[str, Any]:
    """Compact token for browser polling: changes when booking list should refresh."""
    ...
    for booking in bookings:
        ...
        if status_value == BookingStatus.CANCELLED.value:
            cancelled += 1
        changed_at = getattr(booking, "updated_at", None) or getattr(booking, "created_at", None)
        if changed_at and (latest_changed_at is None or changed_at > latest_changed_at):
            latest_changed_at = changed_at
    return {
        "total_bookings": total,
        "cancelled_count": cancelled if cancelled_count is None else cancelled_count,
        "latest_changed_at": _iso_or_none(latest_changed_at),
    }


def _has_ota_changes_since(previous: Optional[Dict[str, Any]], current: Dict[str, Any]) -> bool:
    if not previous:
        return False
    keys = ("total_bookings", "cancelled_count", "latest_changed_at")
...
def ota_dashboard_ui(request: Request):
    """OTA Dashboard cũ đã được chuyển vào Quản lý đặt phòng."""
    return RedirectResponse(url="/pms/booking", status_code=307)
...
def get_ota_stats(
    ...
    latest_changed_at = booking_q.with_entities(
        func.max(func.coalesce(Booking.updated_at, Booking.created_at))
    ).scalar()
...
def update_booking(
    ...
    if payload.status is not None:
        ...
        booking.reservation_status = {
            ...
            "NO_SHOW": "NO_SHOW",
        }.get(status_value, booking.reservation_status or "PENDING")
    service = BookingService(db)
    if is_ota_like_booking(booking) and booking.reservation_status not in {"CANCELLED", "CHECKED_OUT", "NO_SHOW"}:
        service.stage_ota_booking_for_review(booking, user_id=user.get("id"))
    else:
        service.normalize_existing_booking(
            booking,
            user_id=user.get("id"),
            reserve_inventory=booking.reservation_status == "CONFIRMED",
        )
...
def delete_booking(
    ...
    if getattr(booking, "reservation_status", None) == "CONFIRMED":
        BookingService(db).cancel_reservation(booking.id, "Xoá từ OTA Dashboard", user.get("id"), no_show=False)
...
async def _skip_success_message_ids(message_ids: list[str]) -> set[str]:
    from app.db.session import TaskSessionLocal  # type: ignore

    if not message_ids:
        return set()
    db = TaskSessionLocal()
    try:
        rows = db.query(OTAParsingLog.email_message_id).filter(
            OTAParsingLog.email_message_id.in_(message_ids),
...
async def _fetch_gmail_messages(message_ids: list[str], log_prefix: str) -> list[dict]:
    from app.core.config import logger  # type: ignore

    semaphore = asyncio.Semaphore(GMAIL_FETCH_CONCURRENCY)

    async def fetch_one(message_id: str):
        async with semaphore:
            email = await asyncio.to_thread(gmail_service.get_message, message_id)
            if email and gmail_service.is_ota_sender(email.get('sender', '')):
                logger.info(f"[{log_prefix}] ✉️ OTA email: {email.get('sender')} | {email.get('subject')}")
                return email
            if email:
                logger.debug(f"[{log_prefix}] Bỏ qua (không phải OTA): {email.get('sender', '?')}")
            return None
...
async def _process_ota_emails(emails: list[dict], log_prefix: str, processed_at=None, sequential: bool = False) -> tuple[int, int]:
    from app.core.config import logger  # type: ignore
    ...
    async def process_one(email: dict) -> bool:
        async with semaphore:
            ...
            try:
                ...
                logger.error(f"[{log_prefix}] Lỗi xử lý email {email.get('uid')}: {e}")
                db.rollback()
                return False
            finally:
                db.close()
    ...
    if sequential:
        results = []
        for email in emails:
            results.append(await process_one(email))
    else:
        results = await asyncio.gather(*(process_one(email) for email in emails))
    for ok in results:
        if ok:
            processed += 1
        else:
...
async def manual_scan_today(
    ...
    """
    from app.core.config import logger  # type: ignore
    from app.core.utils import VN_TZ  # type: ignore
    ...
    if scan_key in _running_manual_scans:
        logger.info(f"[Manual Scan] Đã có job đang quét ngày {date_str}, bỏ qua yêu cầu trùng.")
        return {"status": "already_running", "message": f"Đang quét email ngày {date_str}; vui lòng chờ job hiện tại hoàn tất.", "scan_date": scan_key}
    ...
    return {"status": "started", "message": f"Đang quét email ngày {date_str} trong nền...", "scan_date": scan_key, "scan_requested_at": scan_requested_at.isoformat()}


def _run_scan_emails_for_date(target_date, scan_requested_at=None, scan_key=None):
    try:
        asyncio.run(_scan_emails_for_date(target_date, scan_requested_at=scan_requested_at))
    finally:
        if scan_key:
            _running_manual_scans.discard(scan_key)


async def _scan_emails_for_date(target_date, scan_requested_at=None):
    """
    Background task: Quét các email OTA trong một ngày cụ thể từ Gmail API.
    target_date: datetime theo ngày địa phương cần quét.
    """
    import asyncio
    from app.core.config import logger  # type: ignore
    ...
    try:
        ...
        pending_ids = [message_id for message_id in message_ids if message_id not in skipped_ids]
        if skipped_ids:
            logger.info(f"[Manual Scan] Bỏ qua {len(skipped_ids)} email đã xử lý thành công trước đó")

        emails = await _fetch_gmail_messages(pending_ids, "Manual Scan")
    ...
    return {
        **status,
        ...
        "setup_guide": {
            ...
            "step4": "Mở /api/ota/oauth/start để cấp quyền Gmail",
            "step5": "Callback sẽ tự đăng ký Gmail Watch; nếu cần có thể gọi POST /api/ota/gmail/watch",
        }
    }
...
def gmail_oauth_start():
    """Bắt đầu OAuth Gmail để cấp lại token khi token cũ hết hạn/thu hồi."""
    from app.core.config import settings  # type: ignore
...
async def gmail_oauth_callback(
    ...
    try:
        ...
        watch_result = gmail_service.watch_inbox()
        watch_message = (
            "Gmail Watch đã được đăng ký thành công."
            if watch_result else
            "Token đã lưu nhưng chưa đăng ký được Gmail Watch. Kiểm tra Pub/Sub topic rồi gọi POST /api/ota/gmail/watch."
        )
        ...
        <body style="font-family:sans-serif; max-width:600px; margin:50px auto; text-align:center;">
            ...
            <p>Token đã được lưu. {watch_message}</p>
