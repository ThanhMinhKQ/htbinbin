...
# [THÊM MỚI] Route xử lý logic đổi mật khẩu
@router.post("/change-password") # Bỏ response_class=HTMLResponse
def change_password_submit(
    ...
    user_session = request.session.get("user")
    if not user_session:
        # Trả về 401 để JS biết đường chuyển hướng
        return JSONResponse(status_code=401, content={"status": "error", "message": "Phiên đăng nhập hết hạn"})
    
    # 1. Validate input
    if new_password != confirm_password:
        return JSONResponse(status_code=400, content={"status": "error", "message": "Mật khẩu xác nhận không khớp."})
    
    if len(new_password) < 6: # Đồng bộ với frontend rule
        return JSONResponse(status_code=400, content={"status": "error", "message": "Mật khẩu quá ngắn (tối thiểu 6 ký tự)."})
    ...
    try:
        ...
        return JSONResponse(status_code=500, content={"status": "error", "message": "Lỗi hệ thống, vui lòng thử lại."})