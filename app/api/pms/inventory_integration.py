...
def get_sellable_products(
    ...
    if warehouse:
        ...
        min_stock_map = {lv.product_id: float(lv.min_stock or 0) for lv in levels}
        ...
        "products": [
            {
                ...
                "min_stock": min_stock_map.get(p.id, p.min_stock_global or 0),
            }
            for p in products
        ],
        "categories": [
...
def consume_inventory(
    ...
    with db.begin():
        ...
        # Batch pre-fetch products + inventory levels (1+1 query thay vì N+N)
        product_ids = [item.product_id for item in payload.items]
        products_all = db.query(Product).filter(
        ...
        inv_levels_all = db.query(InventoryLevel).filter(
            InventoryLevel.warehouse_id == warehouse.id,
            InventoryLevel.product_id.in_(product_ids),
        ).with_for_update().all()
        inv_levels_map = {lv.product_id: lv for lv in inv_levels_all}

        results = []
        low_stock_warnings = []

        for item in payload.items:
            # 3. Get product from pre-fetched map
            ...
            # 4. Check inventory level from pre-fetched map
            inv_level = inv_levels_map.get(product.id)
            ...
            # 7. Create FolioTransaction (skip rebalance — gọi 1 lần cuối loop)
        ...
        # Rebalance folio 1 lần sau tất cả charges (thay vì N lần trong loop)
