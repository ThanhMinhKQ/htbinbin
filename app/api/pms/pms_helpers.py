...
def _room_to_dict(room: HotelRoom, active_stay: Optional[HotelStay] = None) -> dict:
    ...
    d = {
        ...
        "notes": room.notes,
        "condition": room.condition.value if hasattr(room, "condition") and room.condition else "CLEAN",
        ...
        "promo_discount_amount": float(getattr(rt, "promo_discount_amount", 0) or 0) if rt else 0,
        "promo_discount_percent": float(rt.promo_discount_percent) if rt else 0,  # legacy
        "min_hours": rt.min_hours if rt else 1,
        # Standard time frame + hybrid pricing config
        "pricing_config": {
            "std_checkin_time": rt.standard_checkin_time.strftime("%H:%M") if rt and rt.standard_checkin_time else "14:00",
            "std_checkout_time": rt.standard_checkout_time.strftime("%H:%M") if rt and rt.standard_checkout_time else "12:00",
            "early_fee_per_hour": float(rt.early_checkin_fee_per_hour or 0),
            "late_fee_per_hour": float(rt.late_checkout_fee_per_hour or 0),
            "grace_minutes": rt.grace_minutes or 10,
            "day_threshold_hours": rt.day_threshold_hours or 8,
            "price_per_night": float(rt.price_per_night) if rt else 0,
            "price_per_hour": float(rt.price_per_hour) if rt else 0,
            "price_next_hour": float(rt.price_next_hour) if rt else 0,
            "min_hours": rt.min_hours or 1,
        } if rt else None,
...
def _gen_folio_code(db: Session, branch_id: int) -> str:
    return generate_folio_code(db, branch_id)


def _rebalance_folio(db: Session, folio) -> None:
