...
def api_get_rooms(
    ...
    # (100 phòng × 30 stays × 3 guests = 9.000 rows, cần dedup Python)
    # Bây giờ:
    #   Q1: lấy danh sách phòng (flat, 1 row/phòng)
    #   Q2: lấy stays ACTIVE kèm guests (chỉ các phòng có stay active)
    # → Giảm dữ liệu ~90%, không cần dedup, DB plan đơn giản hơn.
    ...
    if target_branch_id:
        rooms_q = rooms_q.filter(HotelRoom.branch_id == target_branch_id)
    rooms = rooms_q.all()

    if not rooms:
        return JSONResponse({"floors": {}, "total_rooms": 0})

    room_ids = [r.id for r in rooms]
    active_stays = (
        db.query(HotelStay)
        .options(selectinload(HotelStay.guests))
        .filter(
    ...
    stay_by_room: dict[int, HotelStay] = {s.room_id: s for s in active_stays}

    result = [_room_to_dict(room, stay_by_room.get(room.id)) for room in rooms]
...
def api_search_rooms(
    ...
    for room in rooms:
        ...
        # Check for promo
        promo_info = None
        promo_amount = float(getattr(rt, "promo_discount_amount", 0) or 0)
        legacy_pct = float(rt.promo_discount_percent or 0)
        if rt.promo_start_time and rt.promo_end_time and (promo_amount > 0 or legacy_pct > 0):
            ci_time = ci.time()
            ...
            if is_promo:
                promo_info = {
                    ...
                    "discount_percent": legacy_pct,  # legacy
