...
def _apply_night_promo(price: Decimal, room_type: HotelRoomType, check_in: datetime) -> Decimal:
    if not (
        room_type.promo_start_time
        and room_type.promo_end_time
        and room_type.promo_discount_percent
        and room_type.promo_discount_percent > 0
    ):
        return price
    ...
    if start <= end:
        if not (start <= ci_time <= end):
            return price
    else:
        if not (ci_time >= start or ci_time <= end):
            return price

    pct = Decimal(str(room_type.promo_discount_percent))
    return _money(price * (Decimal("1") - pct / Decimal("100")))
...
class PricingEngine:
    ...
    def __init__(self, room_type: HotelRoomType):
        ...
        # Overnight Rate — Giá ưu đãi qua đêm (ppn * (1 - promo%))
        # Nếu không có promo → overnight_rate = ppn (không giảm)
        promo_pct = Decimal(str(room_type.promo_discount_percent or 0))
        if promo_pct > 0:
            self.overnight_rate = _money(self.ppn * (Decimal("1") - promo_pct / Decimal("100")))
        else:
            self.overnight_rate = self.ppn
