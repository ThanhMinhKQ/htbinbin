...
class InventoryService:
    ...
    def get_timeline(self, branch_id: int, start_date: date, days: int = 14) -> List[Dict[str, Any]]:
        ...
        for room in rooms:
            ...
            for stay in active_stays_by_room.get(room.id, []):
                ...
                stay_end = check_out_at.date() if check_out_at else end_date
                events.append({
                    "type": "booking",
                    "status": "CHECKED_IN",
                    "label": guest_name,
    ...
    def get_availability(
        ...
        for room_type in q.order_by(HotelRoomType.sort_order, HotelRoomType.name).all():
            inventories = [
                self.get_or_create_inventory(branch_id, room_type.id, d)
                for d in iter_stay_dates(check_in, check_out)
            ]
            min_available = min([i.available_rooms for i in inventories], default=0)
            result.append({
