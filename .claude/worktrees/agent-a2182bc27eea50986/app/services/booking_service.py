...
class BookingService:
    ...
    def _post_booking_deposit_once(self, booking: Booking, user_id: Optional[int]) -> None:
        ...
        if raw.get("deposit_shift_posted") or not booking.deposit_amount or booking.deposit_amount <= 0:
            return
        tx = post_booking_deposit_to_shift(
            ...
            payment_method=booking.payment_method or raw.get("deposit_type") or "Chi nhánh",
    ...
    def list_reservations(
        ...
        total = q.count()
        items = q.order_by(Booking.check_in.asc(), Booking.created_at.desc()).offset((page - 1) * page_size).limit(page_size).all()
        return {
    ...
    def create_reservation(self, payload: Dict[str, Any], user_id: Optional[int]) -> Booking:
        ...
        guest = None
        should_sync_crm_guest = booking_type != "OTA" or bool(payload.get("guest_id")) or bool((payload.get("raw_data") or {}).get("source_ui") == "reservation_wizard")
        if payload.get("guest_id"):
            guest = self.db.query(Guest).filter(
                Guest.id == int(payload["guest_id"]),
                Guest.deleted_at.is_(None),
            ).first()
            if not guest:
                raise ValueError("Không tìm thấy khách CRM đã chọn")
            guest_payload = dict(payload)
            guest_payload["guest_name"] = guest_payload.get("guest_name") or guest.full_name
            guest = self._find_or_create_guest(guest_payload, user_id) or guest
        elif should_sync_crm_guest:
            guest = self._find_or_create_guest(payload, user_id)
        ...
        if payload.get("guest_id"):
            raw_data["guest_id"] = payload.get("guest_id")
