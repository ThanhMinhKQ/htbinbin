...
class OTAExtractor:
    ...
    def extract_data(self, html_content: str, sender: str, subject: str) -> dict:
        ...
        {{
            ...
            "deposit_amount": 0,          // Always 0 for automatic OTA imports; do NOT treat OTA paid amount as a deposit
        ...
        9. For "Go2Joy" bookings: Look for "Tình trạng thanh toán" in the email. If it explicitly says "Chưa thanh toán" or "Thanh toán tại khách sạn", set "is_prepaid": false. If it says "Đã thanh toán" or if the information is missing/not stated, you MUST default to "is_prepaid": true.
        10. IMPORTANT: For all automatic OTA bookings, NEVER create a deposit. Always set "deposit_amount": 0 even if the OTA says paid/prepaid/already paid. The paid/earned amount belongs only in "total_price".
        11. checkin_code: A short numeric/alphanumeric PIN to access the room or building. May appear as "PIN", "Access code", "Check-in code", "Mã check-in". Leave null if not present.
