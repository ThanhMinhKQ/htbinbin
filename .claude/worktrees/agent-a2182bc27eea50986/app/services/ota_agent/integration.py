...
from .rule_extractor import RuleBasedOTAExtractor, is_confident_booking, looks_like_known_non_booking
...
class OTAAgent:
    ...
    def process_email(self, db: Session, mapper: HotelMapper, email: dict, processed_at: Optional[datetime] = None):
        ...
        # 0c. Cache theo booking id trong subject/body: nếu booking đã tồn tại hoặc log SUCCESS rồi thì không parse/Gemini lại.
        cached_external_id = self._guess_external_id(email)
        if cached_external_id and self._already_processed_external_id(db, cached_external_id):
            logger.info(
        ...
        try:
            ...
            data = self.rule_extractor.extract(email)
    ...
    def upsert_booking(self, db: Session, data: dict) -> Booking:
        ...
        if not existing_booking:
            ...
            if action_type == 'CANCEL':
                ...
                existing_booking.check_in = new_check_in or existing_booking.check_in
                existing_booking.check_out = new_check_out or existing_booking.check_out
                ...
                existing_booking.raw_data = self._json_safe(data)
                existing_booking.updated_at = datetime.now()
    ...
    def _normalize_extracted_data(self, data: dict) -> dict:
        ...
        # Rule chung hiện tại: OTA tự động chỉ lưu tổng tiền booking, không ghi/tạo tiền cọc.
