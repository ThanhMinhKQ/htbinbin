    ...
    def retry_failed_email(self, db: Session, log_id: int) -> Dict:
        ...
        try:
            ...
            existing_booking = db.query(Booking).filter(
                Booking.booking_source == extracted.get("booking_source", "Unknown"),
            ...
            if existing_booking:
                ...
                room_type_id = BookingService(db)._resolve_room_type_id(branch_id, room_type)
                if int(extracted.get("num_rooms") or 1) > 1 and room_type_id:
                    payload = {
                        "booking_type": "OTA",
                        ...
                        "guest_name": extracted.get("guest_name") or "Khách OTA",
                        "guest_phone": extracted.get("guest_phone") or "",
                        "check_in": str(extracted.get("check_in"))[:10],
                        "check_out": str(extracted.get("check_out"))[:10],
                        "num_guests": extracted.get("num_guests", 1),
                        "num_adults": extracted.get("num_adults") or extracted.get("num_guests", 1),
                        "num_children": extracted.get("num_children", 0),
                        "total_price": float(extracted.get("total_price") or 0),
                        "currency": extracted.get("currency", "VND"),
                        "is_prepaid": bool(extracted.get("is_prepaid") or False),
                        "payment_method": extracted.get("payment_method"),
                        "deposit_amount": 0,
                        "raw_data": extracted,
                        "room_items": [{
                            "room_type_id": room_type_id,
                            "quantity": int(extracted.get("num_rooms") or 1),
                            "unit_total": 0,
                    ...
                    booking = bookings[0]
                    logger.info(f"[OTA Dashboard] Created grouped booking: {extracted.get('external_id')} x{len(bookings)}")
                else:
                    booking = Booking(
                        booking_source=extracted.get("booking_source", "Unknown"),
                        external_id=extracted.get("external_id"),
                        ...
                        payment_method=extracted.get("payment_method"),
                        # OTA tự động chỉ tạo booking với tổng tiền, không ghi tiền đặt cọc.
                        deposit_amount=0,
