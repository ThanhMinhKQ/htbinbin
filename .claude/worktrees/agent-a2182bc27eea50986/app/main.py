...
@app.middleware("http")
async def clear_jinja_cache(request: Request, call_next):
    """Clear Jinja2 cache after each request to fix Python 3.14 compatibility issue."""
    response = await call_next(request)
    try:
        from .core.templates import clear_template_cache
