...
Object.assign(BookingHub, {
    async scanOtaToday() {
        ...
        try {
            await pmsApi(`/api/pms/reservations/ota/scan-today?${params.toString()}`, { method: 'POST' });
            pmsToast(`Đã bắt đầu quét email OTA ngày ${this.formatDate(scanDate)}`, true);
            setTimeout(() => this.refreshAfterMutation({ availability: true }), 2500);
        } catch (err) {
    ...
    renderOtaStatus(data) {
        ...
        if (windowText) windowText.textContent = `${data.agent_window_days || 7} ngày gần nhất`;
        const latest = document.getElementById('bk-ota-latest');
        if (latest) {
    ...
    buildOtaLogParams() {
        ...
        const modalBranch = (document.getElementById('bk-ota-log-branch-filter')?.value || '').trim();
        const branchId = modalBranch || this.state.branchId;
    ...
    async loadOtaLogs() {
        ...
        if (!rows) return;
        rows.innerHTML = '<tr><td colspan="8">Đang tải log booking/hủy OTA...</td></tr>';
