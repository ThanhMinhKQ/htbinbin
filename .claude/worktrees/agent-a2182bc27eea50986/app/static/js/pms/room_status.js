...
const RoomStatus = {
    ...
    setMode(mode) {
        ...
        document.getElementById('rs-month-panel').style.display = mode === 'month' ? 'block' : 'none';
        this.syncDateControls();
    ...
    renderDayTable(calendar) {
        ...
        calendar.forEach(day => {
            ...
            th.innerHTML = `
                <div class="rs-date-header ${day.date === this.state.selectedDate ? 'active' : ''}">
        ...
        Object.values(typesMap).forEach(type => {
            ...
            calendar.forEach(day => {
                ...
                html += `
                    <td class="rs-avail-cell ${colorClass} ${active}" onclick="RoomStatus.selectDate('${day.date}')">
                        <div class="rs-avail-tile">
        ...
        (dayData.room_types || []).forEach(rt => {
            ...
            html += `
                <div class="rs-room-group ${isOpen ? 'open' : ''}">
                    <div class="rs-room-group-head" onclick="RoomStatus.toggleRoomGroup(this.parentElement)">
                        <span>${this.escape(rt.room_type)} (${availCount} phòng)</span>
                        <i class="ph ph-caret-down"></i>
                    </div>
                    <div class="rs-room-group-body">
                        ${rooms.map(room => `
                            <div class="rs-room-pill">${this.escape(room.room_number)}</div>
    ...
    renderTimeline(rooms, days) {
        ...
        for (let i = 0; i < days; i++) {
            ...
            const isToday = this.toDateInput(d) === this.toDateInput(new Date());
        ...
        grid.innerHTML = sortedRooms.length ? sortedRooms.map(room => {
            ...
            const laneCount = room.is_unassigned ? 1 : Math.max(1, lanes.length);
            ...
            let rowHtml = `
                <div class="${roomClass}" style="--rs-lanes: ${laneCount}">
                    <div class="rs-room-label">
                        <strong>${this.escape(room.room_number)}</strong>
                        <small>${this.escape(room.room_type || '')}</small>
                    </div>
            `;
            ...
            events.forEach(ev => {
                ...
                const range = `${this.formatDay(ev.start_date)} - ${this.formatDay(ev.end_date)}`;
                ...
                rowHtml += `
                    <button class="rs-booking-bar ${status} ${sizeClass} ${ev.type === 'waitlist-group' ? 'waitlist-group' : ''}" 
                            type="button" 
    ...
    prepareTimelineEvents(events, start, days, canGroupWaitlist = false) {
        ...
        if (canGroupWaitlist && waitlistBookings.length) {
            const unique = [...new Map(waitlistBookings.map((booking) => [Number(booking.booking_id), booking])).values()];
            if (unique.length === 1) {
                visible.push(unique[0]);
                return visible;
            }
            const minLeft = Math.min(...unique.map((booking) => booking.left));
            const maxRight = Math.max(...unique.map((booking) => booking.left + booking.width));
            const groupKey = `waitlist-${unique.map((booking) => Number(booking.booking_id)).join('-')}`;
            this.state.waitlistGroups[groupKey] = unique;
            visible.push({
                type: 'waitlist-group',
                status: 'PENDING',
                label: `${unique.length} booking chờ gán`,
                start_date: this.toDateInput(new Date(start.getTime() + minLeft * 86400000)),
                end_date: this.toDateInput(new Date(start.getTime() + (maxRight - 1) * 86400000)),
                left: minLeft,
