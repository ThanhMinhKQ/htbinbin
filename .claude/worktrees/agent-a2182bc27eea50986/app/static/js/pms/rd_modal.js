...
async function rdLoadPayment(autoOpenPayment) {
    ...
    try {
        // Đảm bảo fetchFolio() đã hoàn thành TRƯỚC KHI render
        if (rdFolioLoading) {
            while (rdFolioLoading) await new Promise(r => setTimeout(r, 50));
        }
        
        let shouldFetchPreview = true;

        if (!rdFolioLoaded || rdCurrentFolios.length === 0) {
            await fetchFolio(rdStayData.id);
        ...
        if (shouldFetchPreview) {
            const endpoint = rdIsCheckedOut
                ...
                : `/api/pms/checkout/${rdStayData.id}/preview`;
...
async function fetchFolio(stayId) {
    // Capture session at start to detect stale responses
    const sessionAtStart = _rdSessionId;
    ...
    try {
        ...
        if (rdIsCheckedOut) {
            previewPromise = pmsApi(`/api/pms/checkout/${stayId}/preview-checked-out`).catch(previewErr => {
             ...
             previewPromise = pmsApi(`/api/pms/checkout/${stayId}/preview`).catch(previewErr => {
...
function pmsRdMirrorFolio() {
    ...
    const svList = document.getElementById('rd-services-active-list');
            ...
            return `<div class="rd-mini-list-item">
                <span class="rd-mini-list-name">${pmsEscapeHtml(desc)}${qtyStr}</span>
                <span class="rd-mini-list-val">${pmsMoney(t.amount)}</span>
                ${rdIsCheckedOut || t.is_virtual ? '' : `<button class="rd-mini-delete-btn" onclick="pmsRdVoidTx(${t.id})" title="Xoá dịch vụ">
    ...
    if (surList) {
        ...
        surList.innerHTML = surTxs.map(t => {
            ...
            return `<div class="rd-mini-list-item">
                <span class="rd-mini-list-name">${pmsEscapeHtml(desc)}${qtyStr}</span>
                <span class="rd-mini-list-val">${pmsMoney(t.amount)}</span>
                ${rdIsCheckedOut || t.is_virtual ? '' : `<button class="rd-mini-delete-btn" onclick="pmsRdVoidTx(${t.id})" title="Xoá phát sinh">
                  <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                </button>`}
            </div>`;
        }).join('');
    ...
    try {
        if (hasInventory) {
            ...
            if (legacyServices.length > 0) {
                const legacyResponses = await Promise.all(legacyServices.map(s => {
                    const url = `/api/pms/folio/${rdCurrentFolio.id}/charge?transaction_type=SERVICE_CHARGE&amount=${(s.amount || s.price) * s.qty}&quantity=${s.qty}&description=${encodeURIComponent(s.name)}`;
                    return pmsApi(url, { method: 'POST' });
                }));
                legacyResponses.forEach((legacyResp, idx) => {
                    const s = legacyServices[idx];
            ...
            // ── Legacy: charge trực tiếp không qua kho ──
            const responses = await Promise.all(rdTempServices.map(s => {
                const url = `/api/pms/folio/${rdCurrentFolio.id}/charge?transaction_type=SERVICE_CHARGE&amount=${(s.amount || s.price) * s.qty}&quantity=${s.qty}&description=${encodeURIComponent(s.name)}`;
                return pmsApi(url, { method: 'POST' });
            }));
            responses.forEach((resp, idx) => {
                const s = rdTempServices[idx];
...
async function pmsRdApplySurcharge() {
    ...
    try {
        const responses = await Promise.all(rdTempSurcharges.map(s => {
            const url = `/api/pms/folio/${rdCurrentFolio.id}/charge?transaction_type=SURCHARGE&amount=${s.amount}&quantity=1&description=${encodeURIComponent(s.note)}`;
            return pmsApi(url, { method: 'POST' });
        }));
        responses.forEach((resp, idx) => {
            const s = rdTempSurcharges[idx];
            rdMergeChargeResult(resp, {
