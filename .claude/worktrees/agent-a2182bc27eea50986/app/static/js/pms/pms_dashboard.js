...
document.addEventListener('DOMContentLoaded', () => {
    ...
    PMS_OTA_STATUS.timer = setInterval(() => pmsLoadOtaStatus(true), 60000);
...
async function pmsLoadOtaStatus(silent = false) {
    ...
    try {
        ...
        const pending = Number(data.ota_pending || 0);
        const previous = PMS_OTA_STATUS.lastPending;
        PMS_OTA_STATUS.pending = pending;
        PMS_OTA_STATUS.total = Number(data.ota_total || 0);
...
function pmsRenderOtaStatus(errorMessage) {
    ...
    body.innerHTML = `
        ...
        <div class="pms-ota-metric">
            <span>Tổng OTA</span>
            <strong>${PMS_OTA_STATUS.total || 0}</strong>
        </div>
        <p>${pending > 0 ? 'Có đặt phòng OTA cần kiểm tra và xác nhận.' : 'Không có đặt phòng OTA chờ xác nhận.'}</p>
    `;
}
