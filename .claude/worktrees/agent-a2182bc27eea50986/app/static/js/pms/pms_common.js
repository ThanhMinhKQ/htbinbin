    ...
    async _loadNewProvinces() {
        try {
            ...
            if (data?.provinces) {
                ...
                // Preload wards for major provinces
                const majorProvinces = ['TP HCM', 'Hà Nội', 'Đà Nẵng', 'Cần Thơ', 'Bình Dương'];
                const wardPromises = this.newProvinces
                    .filter(p => majorProvinces.includes(p.short) || p.short.includes('HCM') || p.short.includes('Hà Nội'))
                    .map(p => this._loadNewWards(p.short));
                
                // Don't await all — just start loading
                Promise.all(wardPromises).catch(() => {});
            }
        } catch (e) {
...
// Toast notification
// Toast notification: Premium high-density design
function pmsToast(msg, ok = true) {
    let el = document.getElementById('pms-toast');
    if (!el) {
    ...
    let type = 'info';
    let title = 'Thông báo';
    let icon = `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2a10 10 0 1 0 10 10A10 10 0 0 0 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z"/></svg>`;

    if (ok === true || ok === 'success') {
        type = 'ok';
        title = 'Thành công';
        icon = `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2a10 10 0 1 0 10 10A10 10 0 0 0 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>`;
    } else if (ok === false || ok === 'error') {
        type = 'err';
        title = 'Lỗi hệ thống';
        icon = `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2a10 10 0 1 0 10 10A10 10 0 0 0 12 2zm5 13.59L15.59 17 12 13.41 8.41 17 7 15.59 10.59 12 7 8.41 8.41 7 12 10.59 15.59 7 17 8.41 13.41 12 17 15.59z"/></svg>`;
    } else if (ok === 'warn' || ok === 'warning') {
        type = 'warn';
        title = 'Cảnh báo';
        icon = `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M1 21h22L12 2 1 21zm12-3h-2v-2h2v2zm0-4h-2v-4h2v4z"/></svg>`;
    }
    ...
    el.innerHTML = `
        ...
        <div class="toast-content">
            ...
            <div class="toast-msg">${pmsEscapeHtml(String(msg || ''))}</div>
        </div>
    `;
