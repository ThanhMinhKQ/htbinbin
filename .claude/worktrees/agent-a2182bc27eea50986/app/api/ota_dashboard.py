...
async def manual_scan_today(
    ...
    return {"status": "started", "message": f"Đang quét email ngày {date_str} trong nền...", "scan_date": scan_date or "today", "scan_requested_at": scan_requested_at.isoformat()}
...
async def _scan_emails_for_date(target_date, scan_requested_at=None):
    ...
    try:
        ...
        for i, email in enumerate(emails):
            ...
            try:
                mapper = HotelMapper(db)
                await asyncio.to_thread(ota_agent.process_email, db, mapper, email, scan_requested_at)
                processed += 1
            except Exception as e:
