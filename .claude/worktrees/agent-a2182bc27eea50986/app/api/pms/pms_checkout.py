...
async def api_checkout_preview(
    request: Request,
    stay_id: int,
    discount: str = Query(default="0"),
    extra_charge: str = Query(default="0"),
    ...
    folios_raw = (
        db.query(Folio)
        .options(selectinload(Folio.transactions))
...
async def api_checkout_preview_checked_out(
    request: Request,
    stay_id: int,
    db: Session = Depends(get_db),
):
...
async def api_create_debt_record(
    request: Request,
    stay_id: int,
