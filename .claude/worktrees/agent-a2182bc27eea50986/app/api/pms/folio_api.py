...
async def transfer_transactions(
    request: Request,
    folio_id: int,
...
async def add_payment(
    ...
    import json
    from ...db.models import ShiftReportTransaction, ShiftReportStatus, ShiftPaymentMethod, Branch, TransactionType
    from ...services.shift_report_service import _generate_shift_code
    from ...core.utils import VN_TZ
    ...
    with db.begin():
        ...
        prefix = "Thu nợ PMS" if is_debt else "Thanh toán PMS"
        ...
        try:
            ...
            if chi_nhanh_code:
                ...
                if branch:
                    method_map = {
                        PaymentMethod.CASH: ShiftPaymentMethod.CASH,
                        PaymentMethod.CARD: ShiftPaymentMethod.CARD,
                        PaymentMethod.OTA: ShiftPaymentMethod.OTA,
                        PaymentMethod.COMPANY: ShiftPaymentMethod.UNC,
                        PaymentMethod.BRANCH: ShiftPaymentMethod.BANK_TRANSFER,
                    }
                    shift_pay_method = method_map.get(pay_method, ShiftPaymentMethod.CASH)
                    tx_type_map = {
                        PaymentMethod.CASH: TransactionType.OTHER,
                        PaymentMethod.CARD: TransactionType.CARD,
                        PaymentMethod.OTA: TransactionType.OTA,
                        PaymentMethod.COMPANY: TransactionType.UNC,
                        PaymentMethod.BRANCH: TransactionType.BRANCH_ACCOUNT,
                    }
                    shift_tx_type = tx_type_map.get(pay_method, TransactionType.OTHER)
                    transaction_code_val = _generate_shift_code(db, branch.branch_code or "XX")
                    
                    # Cải thiện thông tin mô tả giao dịch (Không bao gồm pay_method vì đã có cột Loại/Nguồn)
                    transaction_info = f"{prefix} - P.{room_number} - {guest_name} (HĐ: {folio.folio_code})"
                    user_note = meta_payload.get("note") if meta_payload else None
                    if user_note:
                        transaction_info += f" - {user_note}"
...
async def add_payment_with_shift_report(
    request: Request,
    folio_id: int,
        ...
        try:
            ...
            if chi_nhanh_code:
                ...
                if branch:
                    ...
                    # Cải thiện thông tin mô tả giao dịch (Không bao gồm pay_method và đính kèm ghi chú thay vì ghi đè)
                    transaction_info = f"{prefix} - P.{room_number} - {guest_name} (HĐ: {folio.folio_code})"
                    if payload.note:
                        transaction_info += f" - {payload.note}"
                    ...
                    shift_tx = ShiftReportTransaction(
                        ...
                        folio_id=folio.id,
                        folio_transaction_id=tx.id,
                        payment_method=method_map.get(pay_method, ShiftPaymentMethod.CASH),
                        is_auto_posted=True,
                    )
                    db.add(shift_tx)
                    db.flush()
        ...
        try:
            ...
            if chi_nhanh_code:
                ...
                if branch:
                    ...
                    # Cải thiện thông tin mô tả giao dịch
                    transaction_info = f"Hoàn tiền - {reason} - P.{room_number} - {guest_name} (HĐ: {folio.folio_code})"
