...
async def api_get_guest_profile(
    request: Request,
    guest_id: int,
    db: Session = Depends(get_db),
):
...
async def api_get_guest_co_guests(
    request: Request,
    guest_id: int,
    db: Session = Depends(get_db),
):
...
async def api_get_guest_stay_detail(
    request: Request,
    guest_id: int,
    stay_id: int,
    db: Session = Depends(get_db),
...
async def api_get_guest_services(
    request: Request,
    guest_id: int,
    page: int = Query(default=1, ge=1),
    page_size: int = Query(default=20, ge=1, le=100),
...
async def api_get_guest_payments(
    request: Request,
    guest_id: int,
    page: int = Query(default=1, ge=1),
    page_size: int = Query(default=20, ge=1, le=100),
...
async def api_get_guest_timeline_full(
    request: Request,
    guest_id: int,
...
async def api_get_membership_tiers(request: Request, db: Session = Depends(get_db)):
    """
...
async def api_get_crm_membership_settings(request: Request, db: Session = Depends(get_db)):
    _require_login(request)
    return JSONResponse(get_membership_settings(db))


@router.put("/api/pms/crm/settings/membership", tags=["PMS - CRM"])
async def api_update_crm_membership_settings(
    request: Request,
...
async def api_get_guest_membership(
    request: Request,
    guest_id: int,
    db: Session = Depends(get_db),
):
...
async def api_update_guest_blacklist(
    request: Request,
    guest_id: int,
...
async def api_get_guests_by_tier(
    request: Request,
    tier: str,
    page: int = Query(default=1, ge=1),
    page_size: int = Query(default=20, ge=1, le=100),
...
async def api_crm_overview_stats(
    request: Request,
    branch_id: Optional[int] = Query(default=None),
    date_from: Optional[str] = Query(default=None),
    date_to: Optional[str] = Query(default=None),
...
async def api_crm_rebuild_memberships(
    request: Request,
    db: Session = Depends(get_db),
):
    """
