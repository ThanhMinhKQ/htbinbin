...
def _log_deposit_to_shift_report(
    ...
    def gen_code(branch_code: str) -> str:
        while True:
            code = f"{branch_code}-{''.join(random.choices(string.digits, k=5))}"
            if not db.query(ShiftReportTransaction).filter_by(transaction_code=code).first():
                return code
    ...
    tx_type_map = {
        "CASH": "BRANCH_ACCOUNT",
        "Chi nhánh": "BRANCH_ACCOUNT",
        "CARD": "CARD",
        "VNPAY": "CARD",
        "MOMO": "BRANCH_ACCOUNT",
        "BANK_TRANSFER": "BRANCH_ACCOUNT",
        "Chuyển khoản": "BRANCH_ACCOUNT",
        "COMPANY": "COMPANY_ACCOUNT",
        "Công ty": "COMPANY_ACCOUNT",
        "OTA": "OTA",
        "UNC": "UNC",
    }
    tx_type = tx_type_map.get(deposit_type, "BRANCH_ACCOUNT")
    ...
    pm_enum = pm_map.get(payment_method, ShiftPaymentMethod.CASH)

    description = f"Cọc phòng {room_number}"
    if deposit_type and deposit_type != "Chi nhánh":
        description = f"Cọc phòng {room_number} ({deposit_type})"

    new_tx = ShiftReportTransaction(
        transaction_code=gen_code(branch_code),
    ...
    db.add(new_tx)
    logger.info(f"[ShiftReport] Auto-posted deposit: {amount} for stay {stay_id}, room {room_number}, method {pm_enum.value}")
