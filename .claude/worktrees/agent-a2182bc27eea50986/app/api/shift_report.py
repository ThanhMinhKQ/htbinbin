...
from ..db.models import User, ShiftReportTransaction, Branch, Department, ShiftReportStatus, TransactionType, ShiftCloseLog, User, Folio, FolioTransaction, Payment
...
async def edit_shift_transaction_details( # SỬA
    ...
    cascade_folio = False
    if item.folio_transaction_id and item.amount != old_amount:
        folio_tx = db.query(FolioTransaction).filter(FolioTransaction.id == item.folio_transaction_id).first()
        if folio_tx and not folio_tx.is_voided:
            # Cập nhật số tiền (đảo dấu: ShiftReport lưu số dương, Folio lưu số âm)
            folio_tx.amount = money(-item.amount)
            cascade_folio = True
            # Rebalance folio nếu có liên kết
            if folio_tx.folio_id:
                from ..services.folio_service import rebalance_folio
                rebalance_folio(db, folio_tx.folio_id)
...
async def batch_close_transactions(
    ...
    try:
        ...
        try:
            ...
            # ── PMS Revenue: Auto-fill từ các dòng tiền PMS đã kết (is_auto_posted) ──
            if payload.auto_pms:
                pms_revenue_int = int(sum(
                    float(t.amount) for t in closed_transactions
                    if t.is_auto_posted
                ))
            else:
                # Manual input: dùng giá trị từ form
...
async def get_pms_pending_summary(
    ...
    try:
        ...
        result = get_unclosed_pms_checkouts(db, branch_obj.id)
...
async def get_shift_analytics(
    ...
    """
    API Dashboard thống kê doanh thu theo tháng và chi nhánh
    ...
    from ..db.models import ShiftReportTransaction, ShiftCloseLog, ShiftReportStatus, Branch, TransactionType
    ...
    try:
        # --- 1. Tổng hợp doanh thu PMS (từ ShiftCloseLog) ---
        log_query = db.query(ShiftCloseLog).filter(
        ...
        total_pms = float(log_summary.total_pms or 0)
        total_online_closed = float(log_summary.total_online or 0)
        total_branch_closed = float(log_summary.total_branch or 0)
        shift_count = log_summary.shift_count or 0
        total_cash = total_pms - total_online_closed - total_branch_closed

        # --- 2. Thống kê chi tiết theo từng chi nhánh (Bar chart & Table) ---
        branch_ranking = []
        if not chi_nhanh:
            branch_stats = db.query(
                ...
                func.sum(ShiftCloseLog.closed_branch_revenue).label('total_branch'),
                func.count(ShiftCloseLog.id).label('shift_count')
            ).join(ShiftCloseLog.branch).filter(
                extract('month', ShiftCloseLog.closed_datetime) == filter_month,
                extract('year', ShiftCloseLog.closed_datetime) == filter_year
            ).group_by(Branch.branch_code).order_by(desc('total_pms')).all()

            for b in branch_stats:
                pms = float(b.total_pms or 0)
                onl = float(b.total_online or 0)
                br = float(b.total_branch or 0)
                branch_ranking.append({
                    ...
                    "online": onl,
                    "branch_transfer": br,
                    "shift_count": b.shift_count or 0
                })

        # --- 3. Cơ cấu các loại giao dịch (Doughnut Chart) ---
        # Tính toán dựa trên ShiftReportTransaction trạng thái CLOSED
        ...
        type_stats = tx_query.with_entities(
            ShiftReportTransaction.transaction_type,
            func.sum(ShiftReportTransaction.amount).label('total_amount'),
            func.count(ShiftReportTransaction.id).label('tx_count')
        ).group_by(ShiftReportTransaction.transaction_type).all()

        type_breakdown = {}
        total_tx_count = 0
        for stat in type_stats:
            type_name = stat.transaction_type.value if stat.transaction_type else "OTHER"
            type_breakdown[type_name] = float(stat.total_amount or 0)
            total_tx_count += (stat.tx_count or 0)

        # Lấy thêm thông tin DEBT (do DEBT map vào UNC) và OTHER (Tiền mặt)
        total_debt = type_breakdown.get(TransactionType.UNC.value, 0)
        total_ota = type_breakdown.get(TransactionType.OTA.value, 0)
        total_card = type_breakdown.get(TransactionType.CARD.value, 0)
        total_expense = type_breakdown.get(TransactionType.CASH_EXPENSE.value, 0)
        
        # --- 4. Xu hướng theo ngày (Line chart) ---
        # Gom nhóm theo ngày kết ca (closed_datetime)
        ...
        if chi_nhanh:
            daily_query = daily_query.join(ShiftCloseLog.branch).filter(Branch.branch_code == chi_nhanh)
        ...
        return JSONResponse(content={
            "status": "success",
            "data": {
                "period": { "month": filter_month, "year": filter_year, "label": f"Tháng {filter_month}/{filter_year}" },
                "summary": {
